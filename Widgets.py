...
from os import listdir, walk, system, name as OSNAME
...
class QTitleBar(QFrame):
	...
	def toggleOnTop(self):
		...
		self.parent().setWindowFlags((Qt.FramelessWindowHint | Qt.WindowStaysOnTopHint) if not self.isTop else Qt.FramelessWindowHint)
	...
	def maximize(self):
		self.parent().setWindowState(Qt.WindowNoState if self.isMax else Qt.WindowMaximized)
		self.maximizeButton.setText(iconDir["ti-angle-down" if self.isMax else "ti-angle-up"])
...
class Elements(QFrame):
	def __init__(self, p, destination, name):
		...
		self.datas = {}
		self.datas["absolutePath"] = pathJoin(destination, name)
		self.datas["directory"], self.datas["name"] = pathSplit(splitext(self.datas["absolutePath"])[0])
		self.datas["type"] = splitext(self.datas["absolutePath"])[1]
		self.datas["size"] = getRealSize(self.datas["absolutePath"])
		self.datas["ctime"] = time()-getctime(self.datas["absolutePath"])
		self.datas["mtime"] = time()-getmtime(self.datas["absolutePath"])
		self.datas["atime"] = time()-getatime(self.datas["absolutePath"])

		self.setToolTip("\n".join([f"{a}: {b if b!='' and a!='type' else 'Folder'}" for a, b in zip(["Absolute Path", "Parent Directory", "Type", "Size"], list(self.datas.values())[:4])]) + "\n" + "\n".join([f"{a}: {' '.join([f'{c}{d}' if c!=0 else '' for c, d in convertSeconds(b)])} ago" for a, b in zip(["Created", "Modified", "Accessed"], list(self.datas.values())[5:])]))
	...
	def mouseDoubleClickEvent(self, e=None):
		if isdir(self.datas["absolutePath"]):
			self.nav.historyBack.append(self.nav.currentDirectory)
...
def convertSeconds(s):
	s = int(s)
	y , s = divmod(s, 220903200)
	mo, s = divmod(s, 18408600)
	w , s = divmod(s, 604800)
	d , s = divmod(s, 86400)
	h , s = divmod(s, 3600)
	m , s = divmod(s, 60)
	return ((y, "y"), (mo, "M"), (w, "w"), (d, "d"), (h, "h"), (m, "m"), (s, "s"))
