...
class QNavTab(QScrollArea):
	...
	def changeDirectory(self, path=None):
		self.currentDirectory = path if path else self.parent().navTabs[-2].currentDirectory if len(self.parent().navTabs) != 1 else expanduser("~")
		self.displayName = self.currentDirectory if len(self.currentDirectory.split("/")) <= 2 else "/".join(self.currentDirectory.split("/")[-2:])
		self.parent().navSocket.setTabText(self.parent().navTabs.index(self), self.displayName)
		self.parent().navSocket.setTabToolTip(self.parent().navTabs.index(self), self.currentDirectory)
		self.reloadDirectory()
	def reloadDirectory(self):
		class File(QFrame):
			def __init__(self, p, path, name):
				super().__init__(p)
				self.icon = QLabel(iconDir["ti-file" if isfile(path+"/"+name) else "ti-folder"], self)
				self.name = QLabel(name, self)
				self.icon.setGeometry(0, 0, 20, 20)
				self.name.setGeometry(20, 0, self.width()-20, 20)

			def resizeveEvent(self, e=None):
				self.icon.setGeometry(0, 0, 20, 20)
				self.name.setGeometry(20, 0, self.width()-20, 20)
		self.files = []
		l = listdir(self.currentDirectory)
		for i in l:
			if (not self.parent().config["navigation"]["showHidden"] and i[0]!=".") or self.parent().config["navigation"]["showHidden"]:
				self.files.append(i)
		self.files.sort()
		for x, i in enumerate(self.files):
				c = File(self, self.currentDirectory, i)
				c.setGeometry(5, x*20, self.width()-5, 20)
