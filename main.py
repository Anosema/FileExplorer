from PyQt5.QtWidgets import QApplication, QMainWindow, QFrame, QWidget, QLabel, QPushButton, QLineEdit
...
class Main(QMainWindow):
	...
	def reloadStyle(self):
		...
		with open("config.json", "r") as file:
			for x, i in loads(file.read())["scheme"].items():
				c = c.replace(x, i)
		self.setStyleSheet(c)

	def resizeEvent(self, e):
		self.favs.setGeometry(0, 100, int(self.width()/6) if int(self.width()/6) <= 200 else 200 , self.height()-100)

	def keyPressEvent(self, e):
		print(e.text(), e.key(), hex(e.key()))


class FavoriteFrame(QFrame):
	def __init__(self, p):
		super().__init__(p)
		self.reload()
	...
	def reload(self):
		class ShortCut(QWidget):
			def __init__(self, p, text, path, c=0):
				super().__init__(p)
				self.label = QLabel(text, self)
				self.label.setObjectName("P"+str("F" if c == 1 and not path else c if c != 0 else "H"))
				self.path = path
				self.c = c
			def mousePressEvent(self, e):
				print(self.path) if self.path else None
			def resizeEvent(self, e):
				self.label.setGeometry(0, 0, self.geometry().width(), self.geometry().height())
		for i in self.children():
			i.setParent(None)
		with open("config.json", "r") as file: d = loads(file.read())["Fav"]
		self.pins = []
		self.comp = []

		self.pins.append(ShortCut(self, "Pins", None))
		for i, j in d["pins"].items():
			if type(j) == str:
				self.pins.append(ShortCut(self, i, j, 1))
			else:
				self.pins.append(ShortCut(self, i, None, 1))
				for k, l in j.items():
					self.pins.append(ShortCut(self, k, l, 2))

		self.comp.append(ShortCut(self, "Computer", None))
		for i, j in d["computer"].items():
			self.comp.append(ShortCut(self, i, j, 1))
		for i in disk_partitions():
			self.comp.append(ShortCut(self, i[0], i[1], 1))



	def resizeEvent(self, e):
		for x, i in enumerate(self.pins):
			i.setGeometry(0, x*20, self.width(), 20)
		for x, i in enumerate(self.comp):
			i.setGeometry(0, (len(self.pins)+x)*20, self.width(), 20)
