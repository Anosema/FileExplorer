from PyQt5.QtWidgets import QApplication, QTabWidget, QPushButton, QLineEdit
from Widgets import QBorderlessWindow, QTitleBar, QNavTab
...
class Main(QBorderlessWindow):
	def __init__(self):
		super().__init__()
		self.titleBar = QTitleBar(self, "ti-file", "File Explorer")
		self.backInHistoryButton = QPushButton(iconDir["ti-angle-left"], self)
		self.nextInHistoryButton = QPushButton(iconDir["ti-angle-right"], self)
		self.pathLine = QLineEdit(self)
		self.navSocket = QTabWidget(self)
		...
		self.navSocket.setTabBarAutoHide(False)
		self.navSocket.setMovable(True)
		self.newTabButton.clicked.connect(self.addNavTab)
		self.newTabButton.setObjectName("NewTabButton")

		self.backInHistoryButton.clicked.connect(self.goBackInHistory)
		self.nextInHistoryButton.clicked.connect(self.goNextInHistory)
		self.backInHistoryButton.setObjectName("HistoryButton")
		self.nextInHistoryButton.setObjectName("HistoryButton")
	...
	def changeNavTab(self, i):
		self.activeTab = i
		self.pathLine.setText(self.navSocket.widget(self.activeTab).currentDirectory)
		self.updateHistory()
	def updateHistory(self):
		tab = self.navSocket.widget(self.activeTab)
		if tab.historyBack == []:
			self.backInHistoryButton.setEnabled(False)
		else:
			self.backInHistoryButton.setEnabled(True)
		if tab.historyNext == [] :
			self.nextInHistoryButton.setEnabled(False)
		else:
			self.nextInHistoryButton.setEnabled(True)

	def goBackInHistory(self):
		tab = self.navSocket.widget(self.activeTab)	
		tab.historyNext.append(tab.currentDirectory)
		tab.changeDirectory(tab.historyBack.pop())
		self.updateHistory()
	def goNextInHistory(self):
		tab = self.navSocket.widget(self.activeTab)
		tab.historyBack.append(tab.currentDirectory)
		tab.changeDirectory(tab.historyNext.pop())
		self.updateHistory()
	...
	def reloadStyle(self):
		with open("theme.css", "r") as file:
			c = file.read()
		for x, i in self.config["theme"].items():
			c = c.replace(x, i)
	...
	def resizeEvent(self, e=None):
		...
		self.backInHistoryButton.setGeometry(int(.1*self.width()), 80, 20, 20)
		self.nextInHistoryButton.setGeometry(int(.1*self.width())+20, 80, 20, 20)
		self.pathLine.setGeometry(int(.1*self.width())+40, 80, int(.9*self.width())-self.gripSize-40, 20)
		self.navSocket.setGeometry(int(.1*self.width()), 100, int(.9*self.width())-self.gripSize, self.height()-100-self.gripSize)
		self.newTabButton.setGeometry(self.width()-self.gripSize-25, 101, 25, 25)
